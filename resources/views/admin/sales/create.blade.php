...
<form class="card" method="POST" action="{{ route('admin.sales.store') }}" id="sale-form">@csrf
    ...
    <div class="admin-form" style="margin-top:20px"><div class="field"><label for="customer_id">Cliente (opcional)</label><select id="customer_id" name="customer_id"><option value="">Não identificar cliente (Consumidor Final)</option>@foreach ($customers as $customer)<option value="{{ $customer->id }}">{{ $customer->user->name }}</option>@endforeach</select></div><div class="field"><label for="discount">Desconto (Kz)</label><input id="discount" name="discount" type="number" min="0" step="0.01" value="0"></div><div class="field"><label for="payment_method">Forma de Pagamento</label><select id="payment_method" name="payment_method" required>@foreach (['CASH' => 'Numerário (Dinheiro Kz)', 'MULTICAIXA_EXPRESS' => 'Multicaixa Express (MCX)', 'DEBIT_CARD' => 'TPA / Multicaixa Débito', 'BANK_TRANSFER' => 'Transferência Bancária (BAI/BFA)', 'CREDIT_CARD' => 'Cartão de Crédito', 'OTHER' => 'Outro'] as $value => $label)<option value="{{ $value }}">{{ $label }}</option>@endforeach</select></div><div class="field" style="display:flex;align-items:end"><button class="btn" type="submit">Finalizar venda</button></div></div>
</form>
<template id="sale-item-template"><div class="sale-row" style="display:grid;grid-template-columns:1fr 150px auto;gap:10px;align-items:end"><div class="field"><label>Produto</label><select class="sale-product" required><option value="">Selecione um produto</option>@foreach ($products as $product)<option value="{{ $product->id }}" data-price="{{ $product->sale_price }}" data-stock="{{ $product->stock_quantity ?? 0 }}">{{ $product->name }} · estoque {{ $product->stock_quantity ?? 0 }} · {{ number_format((float) $product->sale_price, 2, ',', '.') }} Kz</option>@endforeach</select></div><div class="field"><label>Quantidade</label><input class="sale-quantity" type="number" min="1" value="1" required></div><button class="btn btn-secondary remove-sale-item" type="button">Remover</button></div></template>
<script>
(() => {
